...
@export var sens: Vector2 = Vector2(0.01, 0.125)
...
func _process(delta: float) -> void:
	...
	rot_x.rotation_degrees.x = clampf(rot_x.rotation_degrees.x + (_last_mouse_motion.y * -sens.y) * PI, -85, 85)
...
func controller_camera(delta: float) -> void:
	...
	rot_x.rotation_degrees.x = clampf(rot_x.rotation_degrees.x, -85, 85)
